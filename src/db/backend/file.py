...
class FileDatabase(Database):
    """База данных с хранением в JSON-файлах"""
    ...
    def _load_table(self, table_name: str) -> dict:
        """Загружает таблицу из файла с валидацией структуры"""
        ...
        if "records" not in data:
            raise InvalidFileFormatError(
                f"В файле {table_name} отсутствует поле 'records'"
            )

        if not isinstance(data["columns"], list):
            raise InvalidFileFormatError(
                f"Поле 'columns' в файле {table_name} должно быть списком"
            )

        if not isinstance(data["records"], list):
            raise InvalidFileFormatError(
                f"Поле 'records' в файле {table_name} должно быть списком"
            )
    ...
    def create_table(self, table_name: str, columns: list[str]) -> None:
        ...
        data = {"name": table_name, "columns": columns, "records": []}
    ...
    def get_all_info(self) -> dict[str, tuple[list[str], int]]:
        ...
        for table_name in self.list_tables():
            data = self._load_table(table_name)
            result[table_name] = (data["columns"].copy(), len(data["records"]))
        return result
    ...
    def sort_records(
        ...
        # Преобразуем словари в кортежи для сортировки
